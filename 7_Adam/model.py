...
class deep_feedforward_network(object):
    def __init__(self, n0, hidden_list, activation = 'reLu', drop_out = 0, step_size = 0.1, l2_reg = 0, random_seed = 0, beta = 0.9, optimizerInit = None):
        '''
...
#         np.random.seed(self.seed)
        ...
        self.optimizerInit = optimizerInit
        
        # LAYERS
        self.layers = []
        self.dropout_layers = []
        # first hidden layer
        self.layers.append(linear_layer(self.n0, self.layer_width[0], self.step, self.l2reg, beta = beta, optimizerInit = self.optimizerInit)) 
        # middle layers
        for i in range(1, self.number_layers):
            self.layers.append(linear_layer(self.layer_width[i-1], self.layer_width[i], self.step, self.l2reg, beta = beta,  optimizerInit = self.optimizerInit))
        # output layer
        self.output_layer = logistic_regression_head(self.layer_width[self.number_layers -1], self.step, beta = beta,  optimizerInit = self.optimizerInit)
