...
class linear_layer(object):
    ...
    implement z = WX + b
    '''
    def __init__(self, in_dim, out_dim, step_size = 0.1, l2_reg = 0, beta = 0.9, optimizerInit = None):
        '''
        ...
        self.l2reg = l2_reg
        if optimizerInit:
            self.optimizerW = optimizerInit()
            self.optimizerb = optimizerInit()
    ...
    def backward_prop(self, dz, X):  
        ...
        # calculate velocity
        if self.optimizerW :
            self.vW = self.optimizerW(self.dW)
            self.vb = self.optimizerb(self.db)
        return da

        
    def update(self):
        if not self.optimizerW:
            # regular update
...
class logistic_regression_head(object):
    ...
    '''
    def __init__(self, in_dim, step_size, l2_reg = 0, beta = 0.9,  optimizerInit = None):
        ## random initiate parameters, sample from normal disrtibution
        self.step = step_size
...
def cross_entropy_cost(A, y):
    ...
    return -(np.dot(np.log(A + 1e-10), y.T) + np.dot(np.log(1 - A + 1e-10), (1 - y.T)))/m
